...
async function main() {
  ...
  console.log(`${COLORS.green}+                                       +${COLORS.reset}`);
  console.log(`${COLORS.green}+        INVENTORY RECORDER Rev.2       +${COLORS.reset}`);
  console.log(`${COLORS.green}+         Powered by DENO-DevApp        +${COLORS.reset}`);
  console.log(`${COLORS.green}+++++++++++++++++++++++++++++++++++++++++${COLORS.reset}`);
  ...
  if (args.entry) {
    try {
      ...
      const app = new InventoryRecorder(inputFile);
      
      // Run the recorder and get the result
      locationInfo = await app.run();
      
      console.log(`\n${COLORS.green}🔹 Location Information:${COLORS.reset}`);
      console.log("   Floor:", locationInfo.floor);
      console.log("   Room Name:", locationInfo.room_name);
      console.log(`   Loc1: ${locationInfo.D1}${locationInfo.E1}`);
      console.log("   Loc2:", locationInfo.E2);
      console.log()

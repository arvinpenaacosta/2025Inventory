...
function updateModalStatus(step) {
    const maxScans = parseInt(localStorage.getItem('scanMode')) || DEFAULT_MAX_SCANS;
    if (duplicateMessageBox.style.display === 'none') {
        scanModalTitle.textContent = `Scanning Serial (${step} of ${maxScans})`;
    }
}
...
function stopBatchScan(isCancelled = false) {
    ...
    stopScanner().finally(() => { 
        ...
        if (currentScanCount < maxScans) {
            for (let i = currentScanCount; i < maxScans; i++) {
...
function handleScanSuccess(serial) {
    ...
    currentScanCount++;
    if (currentScanCount > 0 && currentScanCount <= maxScans) {

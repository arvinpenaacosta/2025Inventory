...
class ExtractorThread(QThread):
    ...
    def extract_and_save(self):
        ...
        # Extract data
        self.log_signal.emit(f"🚀 Starting extraction from {self.config['db_name']}.db")
        data = self.extract_table_data(db_path, self.config['sqlite_table'])
        if not data:
            self.log_signal.emit(f"⚠️ No data from table: {self.config['sqlite_table']}")
            return
            
        # Process data
        self.log_signal.emit("🔄 Processing data...")
        processed = self.process_data(data)
        
        # Save to tableData.js
        self.log_signal.emit(f"💾 Saving to {self.config['js_data']}...")
        self.save_to_js(processed, js_file_path)
        
    def extract_table_data(self, db_path, table_name):
        ...
        try:
            ...
            self.log_signal.emit(f"✅ Extracted {len(data)} rows from {table_name}")
            return data
    ...
    def save_to_js(self, data, output_path):
        """Save data to JavaScript file with tableData variable and MD5 hash"""
        try:
            # Compute MD5 hash for data validation
            new_hash = hashlib.md5(json.dumps(data, sort_keys=True, default=str).encode()).hexdigest()
...
// Hash: {new_hash}
const tableData = {json.dumps(data, ensure_ascii=False, indent=2, default=str)};
...
class SQLiteExtractorGUI(QMainWindow):
    ...
    def show_about_dialog(self):
        ...
        about_text = (
            ...
            "to a JavaScript file. It supports automatic monitoring with watchdog and polling, "
            "as well as manual extraction via right-click. Configuration is loaded from a .env file.\n\n"
            "Developed by KindleToolDev - 2025"
        )
        ...
        try:
            if os.path.exists('.env'):
                ...
                self.config = {
                    ...
                    'js_data': os.getenv('JS_DATA', 'tableData.js'),
    ...
    def start_file_monitoring(self):
        ...
        self.observer.start()
        self.log_message(f"🔍 Monitoring {self.config['db_name']}.db for changes")

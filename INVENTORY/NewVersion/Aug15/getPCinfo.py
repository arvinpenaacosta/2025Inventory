...
class EnhancedLocationRecorder:
    ...
    def setup_submit_handler(self):
        ...
        def on_submit():
            ...
            location_data = self.window.answers
            if not location_data or len(location_data) < 5:  # Basic validation
                msg = QMessageBox()
                msg.setWindowTitle("Incomplete Selection")
                msg.setText("Please complete all location selections before submitting.")
                msg.exec_()
                return

            # Start system info collection after location is submitted

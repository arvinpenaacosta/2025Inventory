...
func (e *SQLiteExtractor) extractAndSave() {
	...
	if _, err := os.Stat(queryFile); os.IsNotExist(err) {
		...
		if !e.validateQueryResult(data, "default query") {
			return
		}
		e.logMessage("🔄 Processing data for default query...")
		processed := e.processData(data)
		if e.checkExistingJSFile(jsFilePath, "default query", "DataIn", processed, "1") {
			return
		}
		e.logMessage(fmt.Sprintf("👍👍👍 Saving to %s...", e.config.JSData))
		if err := e.saveToJS(processed, jsFilePath, "default query", "DataIn", true, "1"); err != nil {
			return
		}
		return
	}

...
def load_environment():
    ...
    auto_extract = os.getenv('AUTO_EXTRACT', '30')  # Default to 30 seconds if not specified

    if not all([file_path, file_sqlite, js_path, js_data]):
        raise ValueError("Missing required environment variables. Check your .env file.")

    return file_path, file_sqlite, js_path, js_data, int(auto_extract)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.html_files = []
        self.report_buttons = []
        self.auto_extract_timer = QTimer()
        self.setup_ui()
    ...
    def create_main_actions_group(self, parent_layout):
        ...
        self.auto_extract_switch = QCheckBox("Auto Extract")
        layout.addWidget(self.auto_extract_switch)
    ...
    def setup_connections(self):
        ...
        self.auto_extract_switch.stateChanged.connect(self.toggle_auto_extract)
        self.auto_extract_timer.timeout.connect(self.extract_and_convert_js)

    def load_config(self):
        try:
            self.file_path, self.file_sqlite, self.js_path, self.js_data, self.auto_extract_interval = load_environment()
            self.auto_extract_switch.setText(f"Auto Extract Every {self.auto_extract_interval} Seconds")
    ...
    def toggle_auto_extract(self, state):
        if state == Qt.Checked:
            self.auto_extract_timer.start(self.auto_extract_interval * 1000)  # Convert to milliseconds
            QMessageBox.information(
                self,
                "Auto Extraction Enabled",
                f"Automatic extraction is now enabled.\n"
                f"Extraction will run every {self.auto_extract_interval} seconds until disabled."
            )
            self.status_label.setText(f"Auto extraction enabled (every {self.auto_extract_interval} seconds)")
        else:
            self.auto_extract_timer.stop()
            QMessageBox.information(
                self,
                "Auto Extraction Disabled",
                "Automatic extraction is now disabled.\n"
                "Use the 'Extract & Convert to JavaScript' button for manual extraction."
            )
            self.status_label.setText("Auto extraction disabled")
    ...
    def show_progress(self, show=True):
        ...
        self.auto_extract_switch.setEnabled(not show)
    ...
    def on_extraction_finished(self, success, message, record_count, duration):
        ...
        if success:
            ...
            if not self.auto_extract_switch.isChecked():  # Show message only if auto-extract is off
                QMessageBox.information(
                    self,
                    "✅ Export Successful",
